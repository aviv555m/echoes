...
function showWarning(text) {
  if (!text) { warningBox().style.display = "none"; warningBox().textContent = ""; return; }
...
function saveSettings() {
  ...
  const eye_tracker_enabled = document.getElementById("toggleTracker").checked;
  fetch("/api/settings", {
    method:"POST",
    headers:{"Content-Type":"application/json"},
    body: JSON.stringify({theme, language, eye_tracker_enabled})
  }).catch(()=>{});
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("clearBtn").onclick = () => { inputEl().value = ""; showWarning(""); requestSuggestions(); };
  ...
  document.getElementById("toggleTracker").addEventListener("change", saveSettings);
